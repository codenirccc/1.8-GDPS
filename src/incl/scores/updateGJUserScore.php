...
// rate limiting: at most 5 score updates per account per 10 seconds
$query = $db->prepare("SELECT COUNT(*) FROM actions WHERE type = '9' AND account = :accountID AND timestamp > :time");
$query->execute([':accountID' => $userID, ':time' => $uploadDate - 10]);
if ($query->fetchColumn() >= 5) exit("-1");
...
$old = $query->fetchAll();
$old = $old[0] ?? array('stars' => 0, 'coins' => 0, 'demons' => 0, 'userCoins' => 0, 'diamonds' => 0, 'moons' => 0);

$starsCount = ''; $platformerCount = '';
if(!empty($dinfo)) {
