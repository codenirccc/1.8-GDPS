...
if(!empty($_POST["uncompleted"]) AND $_POST["uncompleted"]==1){
	...
	if($completedLevels == "") $completedLevels = "-1";
...
if(isset($_POST["page"]) AND is_numeric($_POST["page"])){
	$offset = min(max((int)$_POST["page"], 0), $maxPage) * 10;
}else{
	$offset = 0;
}
switch($type){
	...
	case 3: //TRENDING
		...
		break;
	case 5:
		$str = ExploitPatch::number($_POST["str"] ?? "");
		if($str == "") $str = "-1";
	...
	case 19: //unknown but same as map packs (on real GD type 10 has star rated filter and 19 doesn't)
		$order = false;
		if(empty($str)) $str = "-1";
		$str = ExploitPatch::numbercolon($str);
		if(substr_count($str, ",") > $maxSongsPerSearch) $str = "-1";
		$params[] = "levelID IN ($str)";
		break;
	case 11: //AWARDED
		$params[] = "NOT starStars = 0";
