...
class Commands {
	...
	public static function doCommands($accountID, $comment, $levelID) {
		...
		if(substr($comment,0,5) == '!rate' AND $gs->checkPermission($accountID, "commandRate")){
			...
			if(!is_numeric($starStars) OR $starStars < 0 OR $starStars > 10){
				self::createBotComment("Command error: Invalid star count. Usage: !rate <difficulty> <stars> [coins] [featured]", $userID, $levelID);
				return true;
			}
			$starCoins = $commentarray[3] ?? "";
			if($starCoins !== "" AND (!is_numeric($starCoins) OR $starCoins < 0 OR $starCoins > 3)){
				self::createBotComment("Command error: Invalid coin count.", $userID, $levelID);
				return true;
			}
			$starFeatured = $commentarray[4] ?? "";
			if($starFeatured !== "" AND !in_array($starFeatured, [0, 1], true)){
				self::createBotComment("Command error: Invalid featured value.", $userID, $levelID);
				return true;
			}
