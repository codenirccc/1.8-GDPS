...
if (isset($_POST["userName"]) AND isset($_POST["email"]) AND isset($_POST["password"]) AND isset($_POST["repeatPassword"])){
	...
	if (!preg_match('/^[A-Za-z0-9_\- ]{3,15}$/', $userName)) {
		exit("Usernames must be 3-15 characters long and may only contain letters, numbers, spaces, underscores and hyphens.");
	} elseif (strlen($password) < 6 OR strlen($password) > 20) {
		exit("Naughty naughty!");
	} elseif (!filter_var($email, FILTER_VALIDATE_EMAIL) OR strlen($email) > 50) {
		exit("Invalid email address.");
	}
	// IP-based registration rate limiting
	$ip = $gs->getIP();
	$query = $db->prepare("SELECT COUNT(*) FROM actions WHERE type = '33' AND value2 = :ip AND timestamp > :time");
	$query->execute([':ip' => $ip, ':time' => time() - 3600]);
	if ($query->fetchColumn() >= 5) {
		exit("Too many registrations from your IP address. Please try again later.");
	}
	// Checking 
	...
	if ($query->fetchColumn() != 0) {
		...
		if ($password != $repeatPassword) {
			...
			if (isset($_POST["captcha"]) AND $_POST["captcha"] != "" AND ($_SESSION["code"] ?? "") == $_POST["captcha"]) {
				...
				$query = $db->prepare("INSERT INTO actions (type, value, timestamp, value2) VALUES ('33', '1', :time, :ip)");
