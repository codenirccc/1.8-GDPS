...
if(!empty($_POST["userhere"]) AND !empty($_POST["passhere"]) AND !empty($_POST["usertarg"]) AND !empty($_POST["passtarg"]) AND !empty($_POST["levelID"])){
	...
	$passhere = $_POST["passhere"];
	$usertarg = ExploitPatch::remove($_POST["usertarg"]);
	$passtarg = $_POST["passtarg"];
	if(!is_numeric($_POST["levelID"]))
		exit("Invalid levelID");
	$levelID = ExploitPatch::remove($_POST["levelID"]);
	$server = trim($_POST["server"]);
	$parsedurl = parse_url($server);
	if(!isset($parsedurl["host"]) OR !in_array(strtolower($parsedurl["host"]), array_map('strtolower', $allowedTargetServers)))
		exit("The target server is not in the allowed list. Contact the server administrator.");
	$scheme = strtolower($parsedurl["scheme"] ?? "");
	if($scheme != "http" AND $scheme != "https")
		exit("The target server URL must use http or https.");
